...
from .models import RawNote, STAGE_ORDER, StageName, StageStatus
...
class Pipeline:
    ...
    def run(self, project_id: str, start: StageName | str | None = None, stop_at_review: bool = True) -> None:
        ...
        try:
            ...
            for stage in STAGE_ORDER[start_index:]:
                ...
                if state["status"] == StageStatus.COMPLETE and state.get("approved", False):
                    continue
                if state["status"] == StageStatus.REVIEW and not state.get("approved", False):
                    if stop_at_review:
                        return
    ...
    def run_stage(self, project_id: str, stage: StageName | str) -> None:
        ...
        self._set(project_id, stage, StageStatus.RUNNING, progress=0.0, error=None, message="Starting")
        try:
    ...
    def _video(self, project_id: str) -> Path:
        files = [path for path in self.store.artifact(project_id, "source").glob("video.*") if path.is_file()]
        if not files:
    ...
    def _run_calibration(self, project_id: str) -> None:
        calibration = find_keyboard(self._video(project_id), self.store.artifact(project_id, "analysis"))
    ...
    def _run_detection(self, project_id: str) -> None:
        ...
        self.store.write_json(project_id, "analysis/raw-notes.json", [asdict(note) for note in notes])

    def _run_rhythm(self, project_id: str) -> None:
        project = self.store.get(project_id)
        raw = [RawNote(**item) for item in self.store.read_json(project_id, "analysis/raw-notes.json", [])]
    ...
    def _run_score(self, project_id: str) -> None:
        ...
        write_musicxml(score, self.store.artifact(project_id, "score/score.musicxml"), project.title)
    ...
    def _run_validation(self, project_id: str) -> None:
        ...
        score = self.store.read_json(project_id, "score/score.json")
        report = validate_audio(score, self.store.artifact(project_id, "analysis/source.wav"), rendered)
    ...
    def _run_export(self, project_id: str) -> None:
        ...
        safe_name = "".join(character if character.isalnum() or character in "-_" else "_" for character in project.title).strip("_") or "score"
        ...
        with zipfile.ZipFile(archive.with_suffix(".zip.tmp"), "w", zipfile.ZIP_DEFLATED) as package:
            for relative in ("project.json", "score/score.json", "score/score.musicxml", "score/score.mid", "analysis/validation.json"):
